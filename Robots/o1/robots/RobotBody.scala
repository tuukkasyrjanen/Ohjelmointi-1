...
class RobotBody(val world: RobotWorld, initialLocation: GridPos, initialFacing: CompassDir) {
  ...
  def neighboringSquare(direction: CompassDir) :Square = this.world.elementAt(this.location.neighbor(direction))
  ...
  def spinClockwise() = (this.facesTowards = this.facesTowards.clockwise)
