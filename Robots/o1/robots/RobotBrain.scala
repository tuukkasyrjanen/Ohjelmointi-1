...
abstract class RobotBrain(initialName: String, val body: RobotBody) {
  ...
  def squareInFront :Square = this.world.elementAt(this.locationInFront)
  ...
  def robotInFront :Option[RobotBrain] = {
    squareInFront.robot.map(_.brain).flatten
  }
  ...
  def isStuck :Boolean = {
    ...
    for (dir <- 1 to 4){
      if (!mayMoveTowards(tempFacing)){
        returnValue += 1
      }
      tempFacing = tempFacing.clockwise
    }

    if(returnValue == 4) true
    else false
  }
