...
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth + 2, floorHeight + 2) {
  ...
  private var turn = 0
  //private var width = floorWidth
  //private var height = floorHeight
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir) = {
    ...
    elementAt(initialLocation).addRobot(robot)
    robot
  }
  ...
  def addWall(location: GridPos) = this.update(location, Wall)
  ...
  def advanceTurn() = {
    this.nextRobot match {
        case Some(robot)   => robot.takeTurn()
                              if (turn != (this.robots.size-1) ) turn += 1
                              else turn = 0
        case None          =>
