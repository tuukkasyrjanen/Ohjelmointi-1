...
// This companion object provides aConstants and a utility function.
object SnakeGame {
...
class SnakeGame(initialPos: GridPos, initialHeading: CompassDir) {
  ...
  def isOver = {
    ...
    val tempSegments = segments.tail
    val over = tempSegments.contains(segments.head) || !validCoords.contains(head.x) || !validCoords.contains(head.y)
    over
  }
  ...
  def advance() = {
    ...
    if (segments.head.neighbor(snakeHeading) == nextFood) {
      ...
      segments = segments.head.neighbor(snakeHeading) +: segments.dropRight(1)
    }
  }
