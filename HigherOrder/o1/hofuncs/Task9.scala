...
object Task9 extends App {
  ...
  def turnElementsIntoResult(numbers: Vector[Int], initialValue: Int, operation: (Int, Int) => Int) = {
    ...
    for (number <- numbers.indices){
      value = operation(value,numbers(number))
    }
    value
  }
  ...
  def positiveCount(initialNumber: Int, nextNumber: Int) = {
    var posCount = initialNumber
    if (nextNumber > 0) {
      posCount += 1
    }
    posCount
  }
  def productOfNonZeros(initialNumber: Int, nextNumber: Int) = {
