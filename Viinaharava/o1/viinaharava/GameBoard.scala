...
class GameBoard(width: Int, height: Int, boozeCount: Int) extends Grid[Glass](width, height) {
  ...
  private def placeBoozeAtRandom(howMany: Int) = {
    ...
    var n = 0
    while (n != howMany){
      val cord = new GridPos(Random.nextInt(this.width), Random.nextInt(this.height))
      if (!this(cord).isBooze){
        this(cord).pourBooze()
        n += 1
     }
    }
  ...
  def isOutOfWater: Boolean = {
    var ret = true
    for (item <- this.allElements){
      if (!item.isBooze && !item.isEmpty){
        ret = false
      }
    }
    ret
