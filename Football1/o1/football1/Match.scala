...
class Match(val home: Club, val away: Club) {
  ...
  override def toString = {
    var text = ""
    if (isGoalless) {
      text = s"${home.name} vs. ${away.name} at ${home.stadium}: tied at nil-nil"
    }
    else if (isTied) {
      text = s"${home.name} vs. ${away.name} at ${home.stadium}: tied at ${homeGoals}-all"
    }
    else if (goalDifference > 0) {
      text = s"${home.name} vs. ${away.name} at ${home.stadium}: ${homeGoals}-${awayGoals} to ${home.name}"
    }
    else if (goalDifference < 0) {
      text = s"${home.name} vs. ${away.name} at ${home.stadium}: ${awayGoals}-${homeGoals} to ${away.name}"
    }
    text
  }
